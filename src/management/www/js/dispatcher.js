dispatcher_add(function (sammy) {
  ...
  sammy.get("#/", function () {
    ...
    var reqs = {
      overview: {
        path: "/overview",
        options: { ranges: ["lengths-over", "msg-rates-over"] },
      },
      vhosts: "/vhosts",
    };
    if (user_monitor) {
  ...
  sammy.get("#/nodes/:name", function () {
    ...
    render(
      { node: { path: "/nodes/" + name, options: { ranges: ["node-stats"] } } },
      "node",
      "",
    );
  });
  ...
  if (ac.canAccessVhosts()) {
    ...
    sammy.get("#/connections/:name", function () {
      ...
      if (
        connectionDetails.protocol === "AMQP 1-0" ||
    ...
    sammy.get("#/channels/:name", function () {
      render(
        {
          channel: {
            path: "/channels/" + esc(this.params["name"]),
    ...
    sammy.get("#/exchanges/:vhost/:name", function () {
      this.title(
        "Exchange " + esc(this.params["vhost"]) + "/" + this.params["name"],
      );
      var path =
        "/exchanges/" +
        esc(this.params["vhost"]) +
        "/" +
        esc(this.params["name"]);
      render(
        {
          exchange: { path: path, options: { ranges: ["msg-rates-x"] } },
    ...
    sammy.put("#/exchanges", function () {
      if (sync_put(this, "/exchanges/:vhost/:name")) update();
      return false;
    });
    sammy.del("#/exchanges", function () {
      if (sync_delete(this, "/exchanges/:vhost/:name")) go_to("#/exchanges");
      return false;
    });
    ...
    sammy.put("#/queues", function () {
      if (sync_put(this, "/queues/:vhost/:name")) update();
      return false;
    });
    sammy.del("#/queues", function () {
      if (this.params["mode"] == "delete") {
        if (sync_delete(this, "/queues/:vhost/:name")) go_to("#/queues");
    ...
    sammy.post("#/bindings", function () {
      if (
        sync_post(
          this,
          "/bindings/:vhost/e/:source/:destination_type/:destination",
        )
      )
        update();
      return false;
    ...
    sammy.del("#/bindings", function () {
      if (
        sync_delete(
          this,
          "/bindings/:vhost/e/:source/:destination_type/:destination/:properties_key",
        )
      )
        update();
      return false;
    ...
    path(
      "#/vhosts",
      {
        vhosts: { path: "/vhosts", options: { sort: true } },
    ...
    sammy.get("#/vhosts/:id", function () {
      render(
        {
          vhost: {
            path: "/vhosts/" + esc(this.params["id"]),
            options: {
              ranges: ["lengths-vhost", "msg-rates-vhost", "data-rates-vhost"],
            },
          },
          permissions: "/vhosts/" + esc(this.params["id"]) + "/permissions",
          topic_permissions:
            "/vhosts/" + esc(this.params["id"]) + "/topic-permissions",
          users: "/users/",
    ...
    sammy.get("#/users/:id", function () {
      ...
      render(
        {
          user: "/users/" + esc(this.params["id"]),
          permissions: "/users/" + esc(this.params["id"]) + "/permissions",
          topic_permissions:
            "/users/" + esc(this.params["id"]) + "/topic-permissions",
          vhosts: "/vhosts/",
    ...
    sammy.put("#/users-modify", function () {
      ...
      if (is_own_password_change) {
        ...
        if (sync_put(this, "/users/:username")) go_to("#/users");
      }
      return false;
    });
    sammy.del("#/users", function () {
      if (sync_delete(this, "/users/:username")) go_to("#/users");
      return false;
    });
    sammy.put("#/permissions", function () {
      if (sync_put(this, "/permissions/:vhost/:username")) update();
      return false;
    });
    sammy.del("#/permissions", function () {
      if (sync_delete(this, "/permissions/:vhost/:username")) update();
      return false;
    });
    sammy.put("#/topic-permissions", function () {
      if (sync_put(this, "/topic-permissions/:vhost/:username")) update();
      return false;
    });
    ...
    sammy.del("#/policies", function () {
      if (sync_delete(this, "/policies/:vhost/:name")) go_to("#/policies");
      return false;
    });
    sammy.put("#/operator_policies", function () {
      this.params = rename_multifield(
        this.params,
        "definitionop",
        "definition",
      );
    ...
    sammy.del("#/operator_policies", function () {
      if (sync_delete(this, "/operator-policies/:vhost/:name")) update();
    });
    let datamodel = {
      limits: "/vhost-limits",
    ...
    sammy.put("#/limits", function () {
      ...
      if (isNaN(valAsInt)) {
        var e =
          "Validation failed\n\n" +
          this.params.name +
          ' should be a number, actually was "' +
          this.params.value +
          '"';
  ...
  path(
    "#/feature-flags",
    {
      feature_flags: { path: "/feature-flags", options: { sort: true } },
      permissions: "/permissions",
    },
    "feature-flags",
  );
  sammy.put("#/feature-flags-enable", function () {
    if (sync_put(this, "/feature-flags/:name/enable")) update();
    return false;
  });
  path(
    "#/deprecated-features",
    {
      ...
      used_deprecated_features: "/deprecated-features/used",
    },
    "deprecated-features",
  );
