...
function startWithOAuthLogin(oauth) {
  ...
  if (!oauth.logged_in) {
    hasAnyResourceServerReady(oauth, (oauth, warnings) => {
      render_login_oauth(oauth, warnings);
      start_app_login();
    });
  } else {
    start_app_login();
...
function render_login_oauth(oauth, messages) {
  ...
  formatData.declared_resource_servers_count =
    oauth.declared_resource_servers_count;
...
function start_app_login() {
  ...
  } else if (!has_auth_credentials() || !check_login()) {
    app.run();
  }
}
...
function update_warnings() {
  ...
  for (var i = 0; i < feature_flags.length; i++) {
    ...
    if (
      ...
      feature_flag.stability != "experimental"
    ) {
      needs_enabling = true;
    }
  }
  ...
  if (needs_enabling) {
    l1 +=
      '<span>&#9888;</span> All stable feature flags must be enabled after completing an upgrade. <a href="https://www.rabbitmq.com/feature-flags.html">[Learn more]</a>';
  }
  if (needs_deprecating) {
    ...
    l1 +=
      '<span>&#9888;</span> Deprecated features are being used. <a href="https://www.rabbitmq.com/feature-flags.html">[Learn more]</a>';
  }
  l1 += "</p>";
...
function apply_state(reqs) {
  ...
  for (k in reqs) {
    ...
    if (options["vhost"] != undefined && current_vhost != "") {
      ...
      } else req2 = req + "/" + esc(current_vhost);
    } else {
      req2 = req;
    }
    var qs = [];
    ...
    Object.keys(options).forEach(function (key) {
      ...
      if (key == "vhost" || key == "sort" || key == "ranges") return;
...
function show_popup(type, text, _mode) {
  ...
  $("div#outer," + closeButtonCssClass).on("click", function (event) {
    if (
      $(event.target).eq($(closeButtonCssClass)) ||
      !$(event.target).closest(cssClass).length
    ) {
...
function submit_import(form) {
  if (form.file.value) {
    var confirm_upload = confirm(
      "Are you sure you want to import a definitions file? Some entities (vhosts, users, queues, etc) may be overwritten!",
    );
...
function postprocess() {
  $("form.confirm-queue").on("submit", function () {
    return confirm(
      "Are you sure? The queue is going to be deleted. " +
  ...
  $("form.confirm").on("submit", function () {
    return confirm(
      "Are you sure? This object cannot be recovered " + "after deletion.",
    );
  });
  ...
  $("#download-definitions").on("click", function () {
    ...
    var vhost =
      idx <= 0
        ? ""
        : "/" + esc($("select[name='vhost-download'] option:selected").val());
  ...
  $(document).on(
    "click",
    "form.auto-submit select, form.auto-submit input",
    function () {
...
function url_pagination_template_context(
  template,
  context,
  defaultPage,
  defaultPageSize,
  ...
  if (use_regex) {
    ...
    if (!valid_regexp) {
      show_popup(
        "warn",
        fmt_escape_html(
          `Filter expression '${name_request}' is not a valid regular expression, will perform a regular text query`,
        ),
      );
      use_regex = false;
    }
...
function url_pagination_template(template, defaultPage, defaultPageSize) {
  return url_pagination_template_context(
    template,
    template,
    defaultPage,
    defaultPageSize,
...
function renderUsers() {
  render(
    {
      users: {
        path: url_pagination_template("users", 1, 100),
        options: {
          sort: true,
          pagination: true,
        },
...
function renderExchanges() {
  render(
    {
      exchanges: {
        path: url_pagination_template("exchanges", 1, 100),
...
function renderConnections() {
  render(
    {
      connections: {
        path: url_pagination_template("connections", 1, 100),
        options: { sort: true },
      },
    },
    "connections",
...
function renderChannels() {
  render(
    {
      channels: {
        path: url_pagination_template("channels", 1, 100),
        options: { sort: true },
      },
    },
    "channels",
...
function update_pages_from_ui(sender) {
  var val = $(sender).val();
  var raw = !!$(sender).attr("data-page-start")
    ? $(sender).attr("data-page-start")
    : val;
...
function update_multifield(multifield, dict) {
  ...
  type_inputs.each(function (index) {
    ...
    if (type == "list") {
      if (input.length == 1) {
        input.replaceWith(
          '<div class="multifield-sub" id="' + prefix + '"></div>',
        );
      }
      ...
      if (input.length == 1) {
        ...
        $("#" + prefix).replaceWith(multifield_input(prefix, "value", "text"));
      }
    }
  });
  if (!empty_found) {
    ...
    var val_type =
      multifield_input(prefix, "value", "text") +
      " " +
      multifield_input(prefix, "type", t);

    if (dict) {
      multifield.append(
        '<table class="mf"><tr><td>' +
  ...
  if (current_filter_regex_on && $.trim(current_filter).length > 0) {
    try {
      ...
      jElem
        .parents(".filter")
        .append(
          '<p class="status-error">' + fmt_escape_html(e.message) + "</p>",
        );
    }
  }
}
...
function setup_visibility() {
  $("div.section,div.section-hidden").each(function (_index) {
    ...
    var pref = section_pref(current_template, $(this).children("h2").text());
...
function toggle_visibility(item) {
  ...
  if (all.hasClass("section-visible")) {
    if (all.hasClass("section")) store_pref(pref, "f");
    else clear_pref(pref);
...
function update_status(status) {
  var text;
  if (status == "ok") text = "Refreshed " + fmt_date(new Date());
  else if (status == "error") {
    var next_try = new Date(new Date().getTime() + timer_interval);
    text =
      "Error: could not connect to server since " +
      fmt_date(last_successful_connect) +
      ". Will retry at " +
      fmt_date(next_try) +
      ".";
  } else throw "Unknown status " + status;
...
function sync_req(type, params0, path_template, options) {
  ...
  try {
    if (type == "GET") req.send(null);
    else req.send(JSON.stringify(params));
  } catch (e) {
  ...
  if (check_bad_response(req, false)) {
    if (type == "GET") return req.responseText;
    else
  ...
  if ((req.status >= 200 && req.status < 300) || req.status == 1223) {
    ...
    if (
      ...
      if (last_page_out_of_range_error > 0)
        seconds =
          (new Date().getTime() - last_page_out_of_range_error.getTime()) /
          1000;
      if (seconds > 3) {
    ...
    debug(
      "Management API returned status code " +
        req.status +
        " - <strong>" +
        fmt_escape_html_one_line(req.responseText) +
        "</strong>",
    );
    clearInterval(timer);
  }
  ...
  for (i in ids) {
    ...
    if (t == "list") {
      ...
      if (t == "boolean") {
        ...
        var n = parseFloat(v);
        if (isNaN(n)) throw k + " must be a number; got " + v;
        val = n;
      } else {
        val = v;
      }
...
function put_parameter(
  ...
  var params = {
    component: sammy.params.component,
