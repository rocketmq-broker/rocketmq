...
pub async fn dispatch_method(
    ...
    match (method.class_id, method.method_id) {
        ...
        (CLASS_EXCHANGE, METHOD_EXCHANGE_DECLARE) => {
            super::exchange::handle_declare(conn_id, channel, &method.arguments, writer, broker)
                .await;
            true
        }
        (CLASS_EXCHANGE, METHOD_EXCHANGE_DELETE) => {
            super::exchange::handle_delete(conn_id, channel, &method.arguments, writer, broker)
                .await;
            true
        }
        (CLASS_EXCHANGE, METHOD_EXCHANGE_BIND) => {
            super::exchange::handle_bind(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_EXCHANGE, METHOD_EXCHANGE_UNBIND) => {
            super::exchange::handle_unbind(conn_id, channel, &method.arguments, writer, broker)
                .await;
            true
        }

        (CLASS_QUEUE, METHOD_QUEUE_DECLARE) => {
            super::queue::handle_declare(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_QUEUE, METHOD_QUEUE_DELETE) => {
            super::queue::handle_delete(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_QUEUE, METHOD_QUEUE_PURGE) => {
            super::queue::handle_purge(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_QUEUE, METHOD_QUEUE_BIND) => {
            super::queue::handle_bind(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_QUEUE, METHOD_QUEUE_UNBIND) => {
            super::queue::handle_unbind(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }

        (CLASS_BASIC, METHOD_BASIC_CONSUME) => {
            super::basic::handle_consume(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_CANCEL) => {
            super::basic::handle_cancel(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_ACK) => {
            super::basic::handle_ack(conn_id, channel, &method.arguments, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_REJECT) => {
            super::basic::handle_reject(conn_id, channel, &method.arguments, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_NACK) => {
            super::basic::handle_nack(conn_id, channel, &method.arguments, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_GET) => {
            super::basic::handle_get(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_QOS) => {
            super::basic::handle_qos(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }
        (CLASS_BASIC, METHOD_BASIC_RECOVER | METHOD_BASIC_RECOVER_ASYNC) => {
            super::basic::handle_recover(conn_id, channel, &method.arguments, writer, broker).await;
            true
        }

        (CLASS_TX, METHOD_TX_SELECT) => {
            super::tx::handle_tx_select(conn_id, channel, writer, broker).await;
            true
        }
        (CLASS_TX, METHOD_TX_COMMIT) => {
            super::tx::process_tx_commit(conn_id, channel, writer, broker).await;
            true
        }
        (CLASS_TX, METHOD_TX_ROLLBACK) => {
            super::tx::process_tx_rollback(conn_id, channel, writer, broker).await;
            true
        }

        (CLASS_CONFIRM, METHOD_CONFIRM_SELECT) => {
            super::tx::handle_confirm_select(conn_id, channel, &method.arguments, writer, broker)
                .await;
            true
        }
