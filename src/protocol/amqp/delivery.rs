...
const MAX_DELIVERIES_PER_PASS: usize = 100;
...
async fn deliver_round(broker: &Broker) {
    for mut entry in broker.queues.iter_mut() {
        ...
        let consumers = queue.active_consumers.clone();
...
/// Caps at MAX_DELIVERIES_PER_PASS deliveries per pass to avoid starving other queues.
async fn deliver_queue(
    broker: &Broker,
    queue_name: &str,
    queue: &mut crate::queue::state::QueueState,
    consumers: &[(String, u64, u16, bool)],
) {
    ...
    while !queue.messages.is_empty() {
        ...
        let (ref consumer_tag, conn_id, channel, no_ack) = consumers[idx];
        ...
        if !no_ack {
            ...
            let queue_name_arc = queue.name_arc.clone();
            broker
                .delivery_index
                .insert(delivery_tag, queue_name_arc.clone());
            broker
                .conn_deliveries
                .entry(conn_id)
                .or_default()
                .push((queue_name_arc, delivery_tag));
        ...
        if delivered >= MAX_DELIVERIES_PER_PASS {
            break;
        }
    }
}
...
fn next_ready_consumer(
    ...
    for _ in 0..n {
        ...
        let (_, conn_id, channel, _) = &consumers[idx];
...
fn try_send_delivery(broker: &Broker, conn_id: u64, frames: Vec<u8>) -> bool {
    let Some(handle) = broker.connections.get(&conn_id) else {
        return false;
    };
    handle.sink.try_deliver(frames)
}
